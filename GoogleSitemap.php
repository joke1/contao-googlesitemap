...
class GoogleSitemap extends Backend
{
	...
	protected function getPageOptions($pid=0, $domain='', $blnIncludeNews=true)
	{
		...
		if ($blnIncludeNews && in_array('newssitemap', $this->Config->getActiveModules()))
		{
			$arrPages = $this->getNewsOptions($arrPages, $domain);
		}
	...
	public function updateArticle()
	{
		...
		if ($this->Input->get('act') == 'editAll')
			$this->redirect('typolight/main.php?do=article');
		...
		if ($this->Input->post('FORM_SUBMIT') == 'tl_article_update' || $GLOBALS['TL_CONFIG']['noqueryPageLastmod'])
		{
			...
			{
				case 'saveNclose':
					$this->redirect('typolight/main.php?do=article&table=tl_content&id=' . $this->Input->get('id'));
					break;
					
				case 'saveNcreate':
					$this->redirect('typolight/main.php?do=article&table=tl_article&act=create&mode=1&pid=' . $this->Input->get('id'));
					break;
				...
				case 'save':
				default:
					$this->redirect('typolight/main.php?do=article&table=tl_article&act=edit&id=' . $this->Input->get('id'));
			}
		}
...
<input type="submit" name="applyPageUpdate" id="applyPageUpdate" class="tl_submit" alt="update page" accesskey="s" value="' . $GLOBALS['TL_LANG']['MSC']['applyPageUpdate'] . '" />
	...
	public function updateContent()
	{
		...
		if ($this->Input->post('FORM_SUBMIT') == 'tl_content_update' || $GLOBALS['TL_CONFIG']['noqueryPageLastmod'])
		{
			...
			{
				case 'saveNclose':
					$this->redirect('typolight/main.php?do=article&table=tl_content&id=' . $objArticle->id);
					break;
					
				case 'saveNcreate':
					$this->redirect('typolight/main.php?do=article&table=tl_content&act=create&mode=1&pid=' . $this->Input->get('id') . '&id=' . $objArticle->id);
					break;
				...
				case 'save':
				default:
					$this->redirect('typolight/main.php?do=article&table=tl_content&act=edit&id=' . $this->Input->get('id'));
			}
		}
